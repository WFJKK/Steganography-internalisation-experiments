#!/bin/bash
# =============================================================================
# 7B Experiments Block (runs on a single GPU via CUDA_VISIBLE_DEVICES)
# =============================================================================
set -e
...
results_exist() { [ -f "$1" ]; }
log() { echo ""; echo "=== [$(date)] 7B: $1 ==="; }

# -- Sanity check --
log "SANITY CHECK"
head -5 data/acrostics/v3a/train.jsonl > /dev/shm/tiny.jsonl
python3 "${SCRIPT}" stage2 \
    --adapter-dir adapters/acrostics/qwen-7b-stage1 \
    --v0-data /dev/shm/tiny.jsonl \
    --output-dir /dev/shm/tiny-lora \
    --model "${MODEL}" \
    --epochs 1 --batch-size 1 --gradient-accumulation 1 --max-length 512
rm -rf /dev/shm/tiny.jsonl /dev/shm/tiny-lora
echo "7B sanity check passed"

# =============================================================================
# ACROSTIC V3a
# =============================================================================
S1="${REPO}/adapters/acrostics/qwen-7b-stage1"
V3A="${W}/7b-v3a-lora"
V3A_RES="${REPO}/results/acrostics/qwen-7b/v3a_results_200.json"
...
# =============================================================================
V3B="${W}/7b-v3b-lora"
V3B_RES="${REPO}/results/acrostics/qwen-7b/v3b_results_200.json"
...
# =============================================================================
# ACROSTIC V2 6-epoch (compute scaling)
# =============================================================================
V2_6EP="${W}/7b-v2-6ep-lora"
V2_6EP_RES="${REPO}/results/acrostics/qwen-7b/v2_6ep_results_200.json"

if ! adapter_exists "${V2_6EP}"; then
    log "ACROSTIC V2 6-epoch -- Training"
    python3 "${SCRIPT}" stage2 \
        --adapter-dir "${S1}" --v0-data data/acrostics/v2/train.jsonl \
        --output-dir "${V2_6EP}" --model "${MODEL}" \
        --epochs 6 --batch-size ${BS} --gradient-accumulation ${GA} \
        --learning-rate ${LR2} --max-length ${ML} --lora-r ${LR} --lora-alpha ${LA} --resume
fi
if ! results_exist "${V2_6EP_RES}"; then
    log "ACROSTIC V2 6-epoch -- Eval"
    python3 "${SCRIPT}" evaluate-v0 \
        --adapter-dir "${V2_6EP}" --eval-file data/acrostics/v2/test.jsonl \
        --output "${V2_6EP_RES}" --model "${MODEL}" \
        --max-examples ${EVAL_MAX} --temperature ${EVAL_T}
fi
...
# SYNONYM STAGE 1
# =============================================================================
SYN_S1="${W}/7b-syn-stage1-lora"
SYN_S1_RES="${REPO}/results/synonyms/qwen-7b/stage1_results.json"
mkdir -p "${REPO}/results/synonyms/qwen-7b"
...
# =============================================================================
# SYNONYM V0 (partial: 493 train, eval on first 50 of train as proxy)
# =============================================================================
SYN_V0="${W}/7b-syn-v0-lora"
SYN_V0_RES="${REPO}/results/synonyms/qwen-7b/v0_results_partial.json"

if ! adapter_exists "${SYN_V0}"; then
    log "SYNONYM V0 -- Training (493 examples)"
    python3 "${SCRIPT}" stage2 \
        --adapter-dir "${SYN_S1}" --v0-data data/synonyms/v0/train.jsonl \
        --output-dir "${SYN_V0}" --model "${MODEL}" \
        --epochs ${EP} --batch-size ${BS} --gradient-accumulation ${GA} \
        --learning-rate ${LR2} --max-length ${ML} --lora-r ${LR} --lora-alpha ${LA} --resume
fi
if ! results_exist "${SYN_V0_RES}"; then
    log "SYNONYM V0 -- Eval (using train data as proxy, no test set available)"
    python3 "${SYN_EVAL}" v0 \
        --adapter-dir "${SYN_V0}" --eval-file data/synonyms/v0/train.jsonl \
        --output "${SYN_V0_RES}" --model "${MODEL}" \
        --max-examples 50 --temperature ${EVAL_T}
fi
...
print('-' * 60)
acr_dir = 'results/acrostics/qwen-7b'
for name, fname in [
...
print(f'{\"Task\":<28} {\"Exact\":>10} {\"Partial\":>10} {\"EditDist\":>10}')
print('-' * 60)
syn_dir = 'results/synonyms/qwen-7b'
for name, fname in [
    ('Stage 1 (told secret)', 'stage1_results.json'),
    ('V0 (partial, proxy)', 'v0_results_partial.json'),
]:
...
print('=' * 70)
" | tee "${REPO}/results/7b_summary.txt"

log "7B BLOCK COMPLETE"
